...
class CheckpointConf:
    ...
    # if > 0, only the latest max_keep numbered checkpoints are kept on disk
    max_keep: int = 0
    ...
    def save_checkpoint(self, epoch, checkpoint_names=None):
        ...
        if checkpoint_names is None:
            ...
            if not checkpoint_names:
                logging.info("No checkpoint scheduled for this epoch.")
                return
        ...
        # Prune old numbered checkpoints, keeping only the latest max_keep by mtime
        if self.checkpoint_conf.max_keep > 0:
            import glob
            pattern = os.path.join(checkpoint_folder, "checkpoint_*.pt")
            numbered = sorted(glob.glob(pattern), key=os.path.getmtime)
            for old in numbered[: -self.checkpoint_conf.max_keep]:
                os.remove(old)
