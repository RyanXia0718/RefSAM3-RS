...
class SpatialCoverageMasks(Masks):
    """Mask loss augmented with spatial coverage supervision."""
    ...
    def __init__(
        ...
        if self.num_sample_points is not None:
            raise ValueError(
                "SpatialCoverageMasks requires full-mask supervision"
            )
        if not region_grids or any(int(grid) < 1 for grid in region_grids):
            raise ValueError(
                f"region_grids must contain positive integers, got {region_grids}"
            )
        if eps <= 0:
            raise ValueError(f"eps must be positive, got {eps}")
        self.region_grids = tuple(int(grid) for grid in region_grids)
        self.use_occupancy_weight = bool(use_occupancy_weight)
        self.eps = float(eps)

    @staticmethod
    def _occupancy_deficit_weights(target_masks):
        """Compute 1 - foreground/bounding-box fill ratio for each mask."""
        if target_masks.shape[0] == 0:
            return target_masks.new_zeros((0,), dtype=torch.float32)

        target_binary = target_masks > 0.5
        rows = target_binary.any(dim=2)
        cols = target_binary.any(dim=1)
        height, width = target_binary.shape[-2:]

        row_ids = torch.arange(height, device=target_masks.device)[None]
        col_ids = torch.arange(width, device=target_masks.device)[None]
        row_min = torch.where(
            rows, row_ids, torch.full_like(row_ids, height)
        ).amin(dim=1)
        row_max = torch.where(
            rows, row_ids, torch.full_like(row_ids, -1)
        ).amax(dim=1)
        col_min = torch.where(
            cols, col_ids, torch.full_like(col_ids, width)
        ).amin(dim=1)
        col_max = torch.where(
            cols, col_ids, torch.full_like(col_ids, -1)
        ).amax(dim=1)

        foreground_area = target_binary.flatten(1).sum(dim=1).float()
        bbox_area = (
            (row_max - row_min + 1).clamp(min=0)
            * (col_max - col_min + 1).clamp(min=0)
        ).float()
        fill_ratio = foreground_area / bbox_area.clamp(min=1.0)
        occupancy_deficit = (1.0 - fill_ratio).clamp(min=0.0, max=1.0)
        return torch.where(
            foreground_area > 0,
            occupancy_deficit,
            torch.zeros_like(occupancy_deficit),
        )

    def _spatial_coverage_loss(self, src_masks, target_masks, num_boxes):
        if target_masks.shape[0] == 0:
            return src_masks.sum() * 0.0

        target_binary = (target_masks > 0.5).to(dtype=src_masks.dtype)
        probabilities = src_masks.sigmoid()
        if self.use_occupancy_weight:
            occupancy_weight = self._occupancy_deficit_weights(target_masks).to(
                src_masks
            )
        else:
            occupancy_weight = src_masks.new_ones((target_masks.shape[0],))
        per_grid_losses = []

        for grid in self.region_grids:
            target_fraction = F.adaptive_avg_pool2d(
                target_binary[:, None], output_size=(grid, grid)
            )
            covered_fraction = F.adaptive_avg_pool2d(
                (target_binary * probabilities)[:, None],
                output_size=(grid, grid),
            )
            positive_regions = target_fraction > 0
            region_coverage = (
                covered_fraction / target_fraction.clamp(min=self.eps)
            ).clamp(min=self.eps, max=1.0)
            region_loss = -region_coverage.log()
            positive_count = positive_regions.flatten(1).sum(dim=1)
            per_mask_loss = (
                (region_loss * positive_regions).flatten(1).sum(dim=1)
                / positive_count.clamp(min=1)
            )
            per_grid_losses.append(per_mask_loss * occupancy_weight)

        per_mask_loss = torch.stack(per_grid_losses, dim=0).mean(dim=0)
        return per_mask_loss.sum() / num_boxes
    ...
    def get_loss(self, outputs, targets, indices, num_boxes):
        ...
        all_src_masks = outputs["pred_masks"]
        if targets["masks"] is None:
            zero = all_src_masks.sum() * 0.0
            return {
                "loss_mask": zero,
                "loss_dice": zero,
                "loss_spatial_coverage": zero,
            }

        target_masks = (
            targets["masks"] if indices[2] is None else targets["masks"][indices[2]]
        )
        target_masks = target_masks.to(all_src_masks)
        keep = (
            targets["is_valid_mask"]
            if indices[2] is None
            else targets["is_valid_mask"][indices[2]]
        )
        src_masks = all_src_masks[(indices[0], indices[1])][keep]
        target_masks = target_masks[keep]
        ...
        target_masks = target_masks.to(dtype=src_masks.dtype)
        ...
        return {
            ...
            "loss_dice": dice_loss(src_flat, target_flat, num_boxes),
            "loss_spatial_coverage": self._spatial_coverage_loss(
