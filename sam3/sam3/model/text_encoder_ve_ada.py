...
class AdapterTextTransformer(nn.Module):
    ...
    def forward(self, text: torch.Tensor, text_type: Optional[int]) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        if self.output_tokens:
            return pooled, tokens, pooled_hidden
        return pooled
...
class AdapterVETextEncoder(nn.Module):
    ...
    def forward(
        ...
        if isinstance(text[0], str):
            ...
            text_pool, text_memory, text_pool_hidden = self.encoder(tokenized, text_type=text_type)

            assert text_memory.shape[1] == inputs_embeds.shape[1]
            text_attention_mask = text_attention_mask.ne(1)
            text_memory = text_memory.transpose(0, 1)
            text_memory_resized = self.resizer(text_memory)
            text_pool_resized = self.resizer(text_pool_hidden)
        else:
            # Process pre-encoded input
            text_attention_mask, text_memory_resized, tokenized = text
            inputs_embeds = tokenized["inputs_embeds"]
            assert input_boxes is None or len(input_boxes) == 0
            padding = text_attention_mask.to(dtype=torch.bool, device=text_memory_resized.device)
            valid = (~padding).to(dtype=text_memory_resized.dtype).transpose(0, 1).unsqueeze(-1)
            denom = valid.sum(dim=0).clamp(min=1.0)
            text_pool_resized = (text_memory_resized * valid).sum(dim=0) / denom
...
def create_adapter_text_encoder(
    ...
    **kwargs,
    ...
    """Create the adapter-enhanced text encoder used by RefSAM3-RS."""
    if adapter_config is None:
        raise ValueError("adapter_config is required for RefSAM3-RS")
