...
class SAM3VLBackbone(nn.Module):
    ...
    def forward(
        ...
        if self.requires_text_features:
            ...
            output = self.forward_text(
                captions, text_type, input_boxes, additional_text, device
            )
            image_output = self.forward_image(
                samples,
                text_features=output["language_features"],
                text_mask=output["language_mask"],
            )
    ...
    @property
    def requires_text_features(self):
        return getattr(self.vision_backbone, "requires_text_features", False)
    ...
    def _forward_image_no_act_ckpt(
        ...
        vision_output = self.vision_backbone.forward(
            samples, text_features=text_features, text_mask=text_mask
        )
        sam3_features, sam3_pos, sam2_features, sam2_pos = vision_output
        if self.scalp > 0:
    ...
    def _forward_text_no_ack_ckpt(
        ...
        with sdpa_context:
            ...
            text_attention_mask, text_memory, text_embeds = text_output[:3]
            text_pooled = text_output[3] if len(text_output) > 3 else None
        ...
        if text_pooled is not None:
            text_pooled = text_pooled[: len(captions)]
        ...
        if text_pooled is not None:
            output["language_pooled_features"] = text_pooled
