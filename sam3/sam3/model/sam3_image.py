...
class Sam3Image(torch.nn.Module):
    ...
    def _expand_text_output_for_image_batch(
        self,
        text_output: Dict,
        find_input,
        ...
        """Expand unique text features to image-batch order for visual fusion."""
        text_features = text_output["language_features"]
        text_mask = text_output.get("language_mask", None)

        if text_features.dim() != 3:
            raise ValueError(
                "TextScale expects language_features with shape "
                f"[T, B, C] or [B, T, C], got {tuple(text_features.shape)}."
            )

        if text_features.shape[1] == num_unique_texts:
            text_features = text_features.permute(1, 0, 2)
        elif text_features.shape[0] != num_unique_texts:
            raise ValueError(
                "Cannot align TextScale text features with unique text "
                f"batch: features={tuple(text_features.shape)}, "
                f"num_unique_texts={num_unique_texts}."
            )

        if text_mask is not None:
            if text_mask.dim() != 2:
                raise ValueError(
                    "TextScale expects language_mask with shape "
                    f"[B, T], got {tuple(text_mask.shape)}."
                )
            if text_mask.shape[0] != num_unique_texts and text_mask.shape[1] == num_unique_texts:
                text_mask = text_mask.transpose(0, 1)
            if text_mask.shape[0] != num_unique_texts:
                raise ValueError(
                    "Cannot align TextScale text mask with unique text "
                    f"batch: mask={tuple(text_mask.shape)}, "
                    f"num_unique_texts={num_unique_texts}."
                )
            text_mask = text_mask.to(dtype=torch.bool, device=text_features.device)

        img_ids = find_input.img_ids.to(device=text_features.device, dtype=torch.long)
        text_ids = find_input.text_ids.to(device=text_features.device, dtype=torch.long)
        if img_ids.numel() != text_ids.numel():
            raise ValueError(
                "TextScale expected one text id per image query, got "
                f"{img_ids.numel()} image ids and {text_ids.numel()} text ids."
            )
        if text_ids.numel() > 0 and (
            int(text_ids.min().item()) < 0
            or int(text_ids.max().item()) >= text_features.shape[0]
        ):
            raise ValueError(
                "TextScale text_ids are out of range for unique text "
                f"features: max_text_id={int(text_ids.max().item())}, "
                f"num_texts={text_features.shape[0]}."
            )

        # Some collated image batches contain images that are not referenced by the
        # current find stage. Initialize them with a valid text row to avoid
        # all-padding key/value rows in cross-attention; those rows are unused later.
        expanded_features = text_features[:1].expand(
            image_batch_size, -1, -1
        ).clone()
        expanded_mask = None
        if text_mask is not None:
            expanded_mask = text_mask[:1].expand(image_batch_size, -1).clone()

        valid_img = (img_ids >= 0) & (img_ids < image_batch_size)
        if valid_img.any():
            expanded_features[img_ids[valid_img]] = text_features[text_ids[valid_img]]
            if expanded_mask is not None:
                expanded_mask[img_ids[valid_img]] = text_mask[text_ids[valid_img]]

        # Return seq-first features because the neck accepts the original text
        # encoder convention and can unambiguously infer the image batch dimension.
        return expanded_features.permute(1, 0, 2), expanded_mask
    ...
    def forward(self, input: BatchedDatapoint):
        ...
        use_text_guided_visual = getattr(self.backbone, "requires_text_features", False)
        if not use_text_guided_visual:
            backbone_out.update(self.backbone.forward_image(input.img_batch))

        selected_captions = [
            random.choice(text_input["simple_query"])
            for text_input in input.find_text_batch
        ]
        text_type = 2
        text_output = self.backbone.forward_text(
            captions=selected_captions,
            text_type=text_type,
            device=device,
        )

        if use_text_guided_visual:
            image_text_features, image_text_mask = self._expand_text_output_for_image_batch(
                text_output=text_output,
                find_input=input.find_inputs[0],
                image_batch_size=len(input.img_batch),
                num_unique_texts=len(input.find_text_batch),
            )
            backbone_out.update(
                self.backbone.forward_image(
                    input.img_batch,
                    text_features=image_text_features,
                    text_mask=image_text_mask,
                )
            )
        ...
        if find_input.input_points is not None and find_input.input_points.numel() > 0:
            print("Warning: point prompts are ignored in referring image segmentation.")
